...
public abstract class TaskManagerTest<T extends TaskManager> {
    ...
    protected Task createTask() {
        ...
        task.setId(1); // Установим ID для задачи
        return task;
    }
    ...
    protected Subtask createSubtask(Epic epic) {
        Subtask subtask = new Subtask("Description", "Title", Status.NEW, epic.getId(),
                Instant.now(), 0);
        subtask.setId(1); // Установим ID для подзадачи
        return subtask;
    }
    ...
    @Test
    public void shouldReturnNullWhenCreateTaskNull() {
        Task task = manager.addTask(null);
        assertNull(task);
    }

    @Test
    public void shouldReturnNullWhenCreateEpicNull() {
